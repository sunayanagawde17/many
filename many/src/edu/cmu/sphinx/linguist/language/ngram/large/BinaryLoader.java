...
/**
 * Reads a binary language model file generated by the CMU-Cambridge Statistical
...
class BinaryLoader
{
	...
	private void readHeader(DataInputStream stream) throws IOException
	{
		...
		bytesPerIDField = 2; // by default, we consider the file to be a 16-bit
								// LM
		if (version <= 0)
		{ // yes, its the version number
	...
	private void skipBigramsTrigrams(DataInputStream stream) throws IOException
	{
		...
		if (numberBigrams > 0)
		{
			...
			int bytesToSkip = (numberBigrams + 1)
					* LargeTrigramModel.ID_FIELDS_PER_BIGRAM
					* getBytesPerIDField();
		...
		if (numberTrigrams > 0)
		{
			...
			int bytesToSkip = numberTrigrams
					* LargeTrigramModel.ID_FIELDS_PER_TRIGRAM
					* getBytesPerIDField();
			stream.skipBytes(bytesToSkip);
			bytesRead += bytesToSkip;
		}
	 ...
	 * @param bigEndian
	 *            true if the DataInputStream is big-endian, false otherwise
	 * 
			...
			int numberUnigrams, boolean bigEndian) throws IOException
	{
		...
		for (int i = 0; i < numberUnigrams; i++)
		{
			...
			if (unigramID == -1 && i != (numberUnigrams - 1))
				unigramID = i; // nouvelle version sphinx3_lm_convert (3.7) he
								// oui .....
	 ...
	 *            the DataInputStream to read from
	 * @param bigEndian
	 *            true if the DataInputStream is in bigEndian, false otherwise
	 * 
			...
			int numberUnigrams) throws IOException
	{
		...
		System.out.format("%s:%s: %d %d \n",
				...
				Dictionary.SENTENCE_END_SPELLING, startWordID, endWordID);
