...
public abstract class TERpara
{
	...
	public static void getOpts(String[] args)
	{
		...
		{
			System.err.println("Loading " + fn + " as parameter file");
			_para.readPara(fn);
		}

		if (_para.get_string(OPTIONS.REF).equals("") || _para.get_stringlist(OPTIONS.HYP) == null)
		{
	...
	{
		REF, REF_SCORES, HYP, HYP_SCORES, REFLEN, NORMALIZE, CASEON, NOPUNCTUATION, FORMATS, OUTPFX, BEAMWIDTH, SHIFTDIST, SHIFTSIZE, DELETE_COST, INSERT_COST, SUBSTITUTE_COST, MATCH_COST, SHIFT_COST, STEM_COST, SYN_COST, CAP_TER,
		// PHRASE_TABLE,
	...
	public static void init()
	{
		...
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.NEWNORM, "New Normalization (boolean)", new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.USE_AVE_LEN, "Use Average Length (boolean)", new Boolean(
				true));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.SHOW_ALL_REFS, "Show All References (boolean)", new Boolean(
				false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.CASEON, "Case Sensitive (boolean)", new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.NOPUNCTUATION, "Strip Punctuation (boolean)", new Boolean(
				false));
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.REF, "Reference File (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.REF_SCORES, "Reference Scores File (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.STRINGLIST, OPTIONS.HYP, "Hypothesis Files (list)", new String[0]);
		_para.add_opt(Parameter.OPT_TYPES.STRINGLIST, OPTIONS.HYP_SCORES, "Hypothesis Scores Files (list)",
				new String[0]);
		_para.add_opt(Parameter.OPT_TYPES.STRINGLIST, OPTIONS.FORMATS, "Output Formats (list)", Parameter
				.parse_stringlist("all"));
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.OUTPFX, "Output Prefix (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.INTEGER, OPTIONS.BEAMWIDTH, "Beam Width (integer)", new Integer(20));
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.REFLEN, "Reference Length File (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.INTEGER, OPTIONS.SHIFTDIST, "Shift Distance (integer)", new Integer(50));
		_para.add_opt(Parameter.OPT_TYPES.INTEGER, OPTIONS.SHIFTSIZE, "Shift Size (integer)", new Integer(10));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.CAP_TER, "Cap Maximum TER (boolean)", new Boolean(false));
		// _para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.PHRASE_TABLE,
		// "Phrase Table (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.STRING, OPTIONS.ADJUST_PHRASETABLE_FUNC, "Adjust Phrase Table Func (string)",
				"");
		...
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.ADJUST_PHRASETABLE_MIN, "Adjust Phrase Table Min (float)",
				Double.NEGATIVE_INFINITY);
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.ADJUST_PHRASETABLE_MAX, "Adjust Phrase Table Max (float)",
				Double.POSITIVE_INFINITY);
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.USE_PORTER, "Use Porter Stemming (boolean)", new Boolean(
				false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.USE_WORDNET, "Use WordNet Synonymy (boolean)", new Boolean(
				false));
		_para
		...
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.WORD_CLASS_FNAME, "Word Class File (filename)", "");
		_para
				.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.DELETE_COST, "Default Deletion Cost (float)", new Double(
						1.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.STEM_COST, "Default Stem Cost (float)", new Double(1.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.SYN_COST, "Default Synonym Cost (float)", new Double(1.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.INSERT_COST, "Default Insertion Cost (float)",
				new Double(1.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.SUBSTITUTE_COST, "Default Substitution Cost (float)",
				new Double(1.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.MATCH_COST, "Default Match Cost (float)", new Double(0.0));
		_para.add_opt(Parameter.OPT_TYPES.DOUBLE, OPTIONS.SHIFT_COST, "Default Shift Cost (float)", new Double(1.0));
		_para.add_opt(Parameter.OPT_TYPES.PARAMFILE, OPTIONS.PARAM_FILE, "", "");
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.FILTER_PHRASE_TABLE, "Filter Phrase Table (boolean)",
				new Boolean(true));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.DUMP_PHRASETABLE, "Dump Phrase Table (boolean)",
				new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.STRING, OPTIONS.SHIFT_CONSTRAINT, "Shift Constraint (string)", "exact");
		_para.add_opt(Parameter.OPT_TYPES.STRING, OPTIONS.SHIFT_STOP_LIST, "Shift Stop Word List (string)", "");
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.WEIGHT_FILE, "Weight File (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.GENERALIZE_NUMBERS, "Generalize Numbers (boolean)",
				new Boolean(false));
		_para
				.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.NORM_NUMS, "Normalize Numbers (boolean)", new Boolean(
						false));
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.NORM_FILE, "Normalization File (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.NORM_PHRASE_TABLE, "Normalize Phrase Table (boolean)",
				new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.SUM_DUP_PHRASES, "Sum Duplicate Phrases (boolean)",
				new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.FILENAME, OPTIONS.PHRASE_DB, "Phrase Database (filename)", "");
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.IGNORE_MISSING_HYP, "Ignore Missing Hypothesis (boolean)",
				new Boolean(false));
		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.IGNORE_SETID, "Ignore Test Set ID (boolean)", new Boolean(
				false));

		_para.add_opt(Parameter.OPT_TYPES.BOOLEAN, OPTIONS.TRY_ALL_SHIFTS, "Try All Shifts (boolean)", new Boolean(
				false));
		...
		_para.addCmdBoolFlag(OPTIONS.CREATE_CONFUSION_NETWORK, 'C', "true");
		_para.addCmdBoolFlag(OPTIONS.TRY_ALL_SHIFTS, 'A', "true");
