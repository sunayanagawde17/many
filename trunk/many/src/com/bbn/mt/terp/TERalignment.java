...
public class TERalignment
{
	...
	public Comparable<String>[] ref;
	public Comparable<String>[] hyp;
	public Comparable<String>[] aftershift;
	...
	public String toString()
	{
		...
		s += "Reference: " + TERutilities.join(" ", ref) + "\nHypothesis: " + TERutilities.join(" ", hyp)
	...
	public String toPhraseString()
	{
		...
		s += String.format("\nHypErrs: %s\nOtherErr: %.3f\nHypLocMap: %s\n",
				TERutilities.join(" ", "%.3f", getHypEdits()), getHypOthEdits(), TERutilities.join(" ",
						getHypLocMap()));
		if (allshifts == null)
		{
			s += "\nNumShifts: 0";
	...
	public String[][] getAlignmentStrings()
	{
		...
		for (int i = 0; i < alignment.length; i++)
		{
			...
			hind += alignment_h[i];
			rind += alignment_r[i];
			ind += ((alignment_r[i] > alignment_h[i])
					? alignment_r[i]
					: alignment_h[i]);
		}
		return tr;
	...
	public String[][] getSplitPhraseStrings()
	{
		if (alignment != null)
		{
			...
			int pcind = 0;
			ArrayList<Comparable<String>> tmpList = new ArrayList<Comparable<String>>(aftershift.length);
			for (int i = 0; i < alignment.length; i++)
			{
				if (alignment[i] == 'P')
				{
					PhraseTree.phrase_cost pc = costfunc.getPhraseTable()
							.retrieve_exact(ref, rind, alignment_r[i],
									aftershift, hind, alignment_h[i]);
					for (int j = 0; j < alignment_r[i]; j++)
					{
						tmpList.add(ref[rind + j]);
					}
					String ref_ph = TERutilities.join(" ", tmpList);
					tmpList.clear();
					for (int j = 0; j < alignment_h[i]; j++)
					{
						tmpList.add(aftershift[hind + j]);
					}
					String hyp_ph = TERutilities.join(" ", tmpList);
					tmpList.clear();
					tr[pcind][0] = "" + pc.cost;
	...
	public String toDotString()
	{
		...
		for (int i = 0; i < hyp.length; i++)
		{
			...
			if (i > 0)
			{
				s += "nodeOH" + (i - 1) + " -> nodeOH" + i
						+ " [rankdir=\"LR\",shape=\"none\"];\n";
			}
		}
	...
	public double rescore_alignment(TERcost costfunc)
	{
		...
		if (alignment != null)
		{
			...
			for (int i = 0; i < alignment.length; i++)
			{
				...
				{
					case ' ' :
						alignment_cost[i] = costfunc.match_cost(
								aftershift[hind], ref[rind]);
						numEdits += alignment_cost[i];
						break;
					case 'S' :
						alignment_cost[i] = costfunc.substitute_cost(
								aftershift[hind], ref[rind]);
						numEdits += alignment_cost[i];
						break;
					case 'Y' :
						alignment_cost[i] = costfunc.syn_cost(aftershift[hind],
								ref[rind]);
						numEdits += alignment_cost[i];
						break;
					case 'T' :
						alignment_cost[i] = costfunc.stem_cost(
								aftershift[hind], ref[rind]);
						numEdits += alignment_cost[i];
						break;
					case 'I' :
						alignment_cost[i] = costfunc
								.insert_cost(aftershift[hind]);
						numEdits += alignment_cost[i];
						break;
					case 'D' :
					...
					case 'P' :
						...
						{
							PhraseTree.phrase_cost pc = pt.retrieve_exact(ref,
									rind, alignment_r[i], aftershift, hind,
									alignment_h[i]);
		...
		if (alignment != null)
		{
			...
			for (int i = 0; i < alignment.length; i++)
			{
				switch (alignment[i])
				{
					...
					case 'Y' :
					case 'T' :
						costfunc.get_wgt_edit(tr, alignment[i],
								aftershift[hind], ref[rind]);
						break;
					case 'I' :
						costfunc.get_wgt_edit(tr, alignment[i],
					...
					case 'D' :
						costfunc
								.get_wgt_edit(tr, alignment[i], null, ref[rind]);
						break;
					case 'P' :
						costfunc.get_wgt_phrase(tr, aftershift, hind,
								alignment_h[i], ref, rind, alignment_r[i]);
						break;
				}
				hind += alignment_h[i];
				rind += alignment_r[i];
	...
	public static void performShiftArray(HashMap<String, ArrayList<Integer>> hwords, int start, int end,
			int moveto, int capacity)
	{
		...
		else
		{
			...
			copyHashWords(hwords, nhwords, end + moveto - start + 1, capacity,
					end + moveto - start + 1);
		}
		hwords.clear();
	...
	private String getPraStr(Comparable<String>[] ref, Comparable<String>[] aftershift,
			...
			for (int i = 0; i < alignment.length; ++i)
			{
				...
				if (alignment[i] != 'D')
				{
					...
					if (slen > 0)
					{
						...
						if (val != null)
							shift_in_str = TERsgml.join(",", (ArrayList<Integer>) val);
	...
	private static void copyHashWords(HashMap<String, ArrayList<Integer>> ohwords, HashMap<String, ArrayList<Integer>> nhwords,
	...
	private void calc_hyp_err()
	{
		...
		for (int i = 0; i < alignment_h.length; i++)
		{
			...
			if (alignment_h[i] == 0)
				ref_ocost += alignment_cost[i];
			if (alignment_h[i] > 0)
			{
				double delta = this.alignment_cost[i] / alignment_h[i];
				for (int j = 0; j < alignment_h[i]; j++)
