...
public class PhraseTable
{
	...
	/**
	 * Returns an ArrayList that contains ArrayLists of valid phrases. Each of
	...
	{
		set_adjust_func(func, params, Double.NEGATIVE_INFINITY,
				Double.POSITIVE_INFINITY, null);
	}
	public void set_adjust_func(String func, double[] params, double min,
			double max)
	{
		set_adjust_func(func, params, min, max, null);
	}
	public void set_adjust_func(String func, double[] params, double min,
			double max, TERcalc tc)
	{
			...
			Comparable[] seq_a, Comparable[] seq_b)
	{
		...
		switch (adjust_func)
		{
			case NONE :
				break;
			/*
			 * case LIN: newcost = adjust_params[0] + (adjust_params[1] *
			 * (adjust_params[2] + cost)); break; case LIN_NED: newcost =
			 * adjust_params[0] + (adjust_params[1] * (adjust_params[2] + cost)
			 * * (adjust_params[3] + numedit)); break; case LOG: newcost =
			 * adjust_params[0] + (adjust_params[1] * ln(adjust_params[2] *
			 * (adjust_params[3] + cost))); break; case LOG10: newcost =
			 * adjust_params[0] + (adjust_params[1] * log10(adjust_params[2] *
			 * (adjust_params[3] + cost))); break; case LOG_NED: newcost =
			 * adjust_params[0] + (adjust_params[1] * ln(adjust_params[2] *
			 * (adjust_params[3] + cost)) * (adjust_params[4] + numedit));
			 * break; case LOG10_NED: newcost = adjust_params[0] +
			 * (adjust_params[1] * log10(adjust_params[2] * (adjust_params[3] +
			 * cost)) * (adjust_params[4] + numedit)); break; case STDLOG:
			 * newcost = adjust_params[0] + (((adjust_params[1] * log10(1.0 -
			 * cost)) + adjust_params[2]) * (adjust_params[3] + numedit));
			 * break; case STDLOG2: newcost = adjust_params[0] +
			 * (((adjust_params[1] * log10(cost)) + adjust_params[2]) *
			 * (adjust_params[3] + numedit)); break;
			 */
			case STD :
				newcost = adjust_params[0]
			...
			case STDINV :
				...
				break;
			case STDNONED :
				newcost = adjust_params[0] + (adjust_params[1] * log10(cost))
			...
			case STDINVNONED :
				...
				break;
			/*
			 * case NED: newcost = adjust_params[0] + (adjust_params[1] *
			 * (adjust_params[2] + numedit)); break; case NED_BINS: if
			 * (adjust_params == null) break; for (i = 0; (i+1) <
			 * adjust_params.length; i+=2) { if (numedit < adjust_params[i]) {
			 * newcost = adjust_params[i+1]; i = adjust_params.length; } } if (i
			 * < adjust_params.length) newcost = adjust_params[i]; break; case
			 * LENGTH_BINS: if (adjust_params == null) break; for (i = 0; (i+1)
			 * < adjust_params.length; i+=2) { if (maxlen < adjust_params[i]) {
			 * newcost = adjust_params[i+1]; i = adjust_params.length; } } if (i
			 * < adjust_params.length) newcost = adjust_params[i]; break;
			 */
		}
		phcost.cost = newcost;
		phcost.numedits = numedit;
		if (phcost.cost > adjust_max)
			phcost.cost = adjust_max;
		if (phcost.cost < adjust_min)
			phcost.cost = adjust_min;
		return phcost;
	}
	...
	private int insert(Set<PhraseEntry> fullset)
	{
		...
		{
			boolean added = insert(pe.getRefWds(), pe.getHypWds(), pe.getProb());
			if (added)
				num++;
		}
		return num;
			...
			PhraseTree.phrase_cost cost)
	{
		...
		{
			seq_a = adjust_tercalc.getCostFunc().process_input_ref(
					(String[]) seq_a);
			seq_b = adjust_tercalc.getCostFunc().process_input_hyp(
					(String[]) seq_b);
		}
	...
	private static class adjust_func_spec
	{
		public adjust_func_spec(ADJUST_FUNC func, String name,
				boolean need_nedits, int num_params, String formula)
		{
			this.func = func;
			this.name = name;
		...
		public String toString()
		{
			String tr = null;
			if (num_params >= 0)
				tr = name + " adjust function needs " + num_params
						+ " parameters.\n";
			else
				tr = name
						+ " adjust function can take any number of parameters.\n";
	...
	static
	{
		adjust_func_names.put("NONE", new adjust_func_spec(ADJUST_FUNC.NONE,
				"NONE", false, 0, "NEWCOST = %COST%"));
		/*
		 * adjust_func_names.put("LIN", new adjust_func_spec(ADJUST_FUNC.LIN,
		 * "LIN", false, 3, "NEWCOST = a + (b * (c + %COST%))"));
		 * adjust_func_names.put("LOG", new adjust_func_spec(ADJUST_FUNC.LOG,
		 * "LOG", false, 4, "NEWCOST = a + (b * log(c * (d + %COST%)))"));
		 * adjust_func_names.put("LOG10", new
		 * adjust_func_spec(ADJUST_FUNC.LOG10, "LOG10", false, 4,
		 * "NEWCOST = a + (b * log_10(c * (d + %COST%)))"));
		 * adjust_func_names.put("LIN_NED", new
		 * adjust_func_spec(ADJUST_FUNC.LIN_NED, "LIN_NED", true, 4,
		 * "NEWCOST = NEWCOST = a + (b * (c + %COST%) * (e + %NED%))"));
		 * adjust_func_names.put("LOG_NED", new
		 * adjust_func_spec(ADJUST_FUNC.LOG_NED, "LOG_NED", true, 5,
		 * "NEWCOST = a + (b * log(c * (d + %COST%)) * (e + %NED%))"));
		 * adjust_func_names.put("LOG10_NED", new
		 * adjust_func_spec(ADJUST_FUNC.LOG10_NED, "LOG10_NED", true, 5,
		 * "NEWCOST = a + (b * log_10(c * (d + %COST%)) * (e + %NED%))"));
		 * adjust_func_names.put("NED", new adjust_func_spec(ADJUST_FUNC.NED,
		 * "NED", true, 3, "NEWCOST = a + (b * (c + %NED%))"));
		 * adjust_func_names.put("LENGTH_BINS", new
		 * adjust_func_spec(ADJUST_FUNC.LENGTH_BINS, "LENGTH_BINS", false, -1,
		 * " [a1, a2, b1, b2, ... n1, n2, o]\n" +
		 ...
		 * "   to the original cost")); adjust_func_names.put("NED_BINS", new
		 * adjust_func_spec(ADJUST_FUNC.NED_BINS, "NED_BINS", true, -1,
		 * " [a1, a2, b1, b2, ... n1, n2, o]\n" +
		 ...
		 * 
		 * adjust_func_names.put("STDLOG", new
		 * adjust_func_spec(ADJUST_FUNC.STDLOG, "STDLOG", true, 4,
		 * "NEWCOST = a + (((b * log_10(1.0 -%COST%)) + c) * (d + %NED%))"));
		 * adjust_func_names.put("STDLOG2", new
		 * adjust_func_spec(ADJUST_FUNC.STDLOG2, "STDLOG2", true, 4,
		 * "NEWCOST = a + (((b * log_10(%COST%)) + c) * (d + %NED%))"));
		 */
		adjust_func_names.put("STD", new adjust_func_spec(ADJUST_FUNC.STD,
				"STD", true, 4, "NEWCOST = a + (b * %NED% * log_10(%COST%))"
						+ " + (c * %NED% * %COST%) + (d * %NED%)"));
		adjust_func_names.put("STDINV", new adjust_func_spec(
				ADJUST_FUNC.STDINV, "STDINV", true, 4,
				"NEWCOST = a + (b * %NED% * log_10(1.0 - %COST%))"
						+ " + (c * %NED% * (1.0 - %COST%)) + (d * %NED%)"));
		adjust_func_names.put("STDNONED", new adjust_func_spec(
				ADJUST_FUNC.STDNONED, "STDNONED", false, 3,
				"NEWCOST = a + (b * log_10(%COST%))" + " + (c * %COST%)"));
		adjust_func_names.put("STDINVNONED", new adjust_func_spec(
				ADJUST_FUNC.STDINVNONED, "STDINVNONED", false, 3,
	...
	public int fullTreeNumNodes()
	{
		return fullTree.num_nodes();
	}
	public int curTreeNumNodes()
	{
		return curTree.num_nodes();
	}
	public int fullTreeNumTerms()
	{
		return fullTree.num_terms();
	}
	public int curTreeNumTerms()
	{
		return curTree.num_terms();
	}
	public int fullTreeNumEntries()
	{
		return fullTree.num_entries();
	}
	public int curTreeNumEntries()
	{
		return curTree.num_entries();
	}
	public int fullTreeDepth()
	{
		return fullTree.depth();
	}
	public int curTreeDepth()
	{
		return curTree.depth();
	}
			...
			int hstart, int hlen, Comparable[] ref, int rstart, int rlen)
	{
		...
		switch (adjust_func)
		{
			...
			default :
				System.out
						.println("Warning.  Current adjustment function cannot be properly counted.");
				break;
		}
		return;
	}
