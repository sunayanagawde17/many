...
#my $_RELEASE_DIR="/Users/barrault/Documents/workspace/BIN";
...
# TERp parameters
my ($_DEL_COST, $_STEM_COST, $_SYN_COST, $_INS_COST, $_SUB_COST, $_MATCH_COST, $_SHIFT_COST) = (undef, undef, undef, undef, undef, undef, undef);
...
$_NB_SYS = @_HYPOTHESES;
die "Please, specify at least two inputs for system combination ...\n" if($_NB_SYS < 2);
die "Please, specify a working directory ...\n" if !defined($_WORKING_DIR);

#dump_vars();

print STDOUT "MANY - start time is : ".`date`."\n";
...
my @cfg =();
push(@cfg, ("--config-type", "MANY", "--nbsys", "$_NB_SYS", "--output", "$_OUTPUT"));
...
print STDOUT "CMD : java -Xmx10G -Dfile.encoding=".$enc." -cp $_MANY edu.lium.mt.MANY $_MANY_CONFIG\n";
print STDOUT "Starting MANY system combination ...";
print STDOUT "\n------- START LOG ----\n";
safesystem("java -Xmx10G -Dfile.encoding=$enc -cp $_MANY edu.lium.mt.MANY $_MANY_CONFIG");
print STDOUT "\n------- END LOG ----\n";
print STDOUT " OK \n";
...
--null-penalty <null penalty>          : default is $_NULL_PEN \
--word-penalty <word penalty>          : default is $_WORD_PEN \
--multithread <number of thread>       : default is $_NB_THREADS \
